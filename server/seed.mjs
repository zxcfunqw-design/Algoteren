export const tasks = [
  ...
  },
  {
    ...
    timeLimit: "2 seconds",
    memoryLimit: "256 MB",
    statement: "Given two code sketches for the same counting task, determine which approach fits the constraints and implement the accepted one.",
  ...
  },
  {
    ...
    timeLimit: "2 seconds",
    memoryLimit: "256 MB",
    statement: "For each query, construct the largest possible value after applying independent bit decisions over a range of candidates.",
  ...
  },
  {
    ...
    timeLimit: "1 second",
    memoryLimit: "256 MB",
    statement: "Each bank has a balance a[i]. One transfer consumes x units from one bank. Find the maximum x so that at least k transfers can be made.",
  ...
  },
  {
    ...
    timeLimit: "1 second",
    memoryLimit: "256 MB",
    statement: "Given a box A by B by C, find the largest integer cube side s such that at least n cubes of side s fit inside.",
  ...
  },
  {
    ...
    timeLimit: "2 seconds",
    memoryLimit: "256 MB",
    statement: "Find the longest segment whose total difficulty is at most S while each adjacent pair keeps the weekly monotonic training rule.",
...
export const articles = [
  {
    ...
    sections: [
      ...
      { heading: "Use tight language", body: "O(n) is an upper bound, Theta(n) is a tight bound, and Omega(n) is a lower bound." },
      { heading: "C++ fast input template", body: "For large tests, switch off sync and untie streams before reading input.", code: "ios::sync_with_stdio(false);\ncin.tie(nullptr);" }
    ]
  },
  {
    id: "idea-greedy",
    ...
    sections: [
      { heading: "Greedy needs a reason", body: "A greedy solution needs a proof that an optimal answer can be transformed to include the greedy choice." },
      { heading: "Common pattern", body: "Sort events, process them left to right, and keep available choices in a heap." },
      { heading: "Stress testing", body: "Use Python to compare brute force against greedy on random small tests.", code: "for n in range(1, 9):\n    for seed in range(500):\n        test = generate(seed, n)\n        assert brute(test) == greedy(test)" }
    ]
  },
  {
    ...
    sections: [
      { heading: "Predicate first", body: "Before writing the loop, define can(x). If can(x) is monotonic, binary search becomes mechanical." },
      { heading: "Template", body: "This version finds the maximum x for which can(x) is true.", code: "long long lo = 0, hi = 1e18;\nwhile (lo < hi) {\n    long long mid = lo + (hi - lo + 1) / 2;\n    if (can(mid)) lo = mid;\n    else hi = mid - 1;\n}" }
    ]
  },
  {
    id: "analysis-surfing",
    ...
    sections: [
      { heading: "Key observation", body: "A power-up before an obstacle can be used later, so we delay using it until the obstacle cannot be crossed." },
      { heading: "Implementation", body: "Push power-ups into a max-heap. For each obstacle, pop the strongest available power-up until the jump is large enough.", code: "while (!power_ups.empty() && current_power < need) {\n    current_power += power_ups.top();\n    power_ups.pop();\n}" }
    ]
  },
  {
    id: "analysis-binary",
    ...
    sections: [
      { heading: "Predicate", body: "For fixed x, bank i can make floor(a[i] / x) transfers. If the total is at least k, x is feasible." },
      { heading: "Takeaway", body: "When bigger answers make the condition harder, use binary search for the maximum true value." }
    ]
  },
  {
    ...
    sections: [
      { heading: "Window invariant", body: "The right pointer expands the segment. The left pointer moves only when the sum becomes too large or the rule breaks." },
      { heading: "Why it is linear", body: "Each pointer moves at most n times, so the whole scan is O(n)." }
    ]
  }
];
...
export const contests = [
  { id: "monthly-may", title: "AlgoTeren Monthly Contest", status: "Active", window: "May 30, 19:00-21:30", taskCount: 4, participants: 86, topScore: 3200 },
  { id: "prep-2", title: "AlgoTeren Prep Contest #2", status: "Finished", window: "May 8", taskCount: 4, participants: 54, topScore: 2800 },
  { id: "binary-basics", title: "NIS Binary Basics Sprint", status: "Upcoming", window: "June 6", taskCount: 4, participants: 41, topScore: 0 }
];
